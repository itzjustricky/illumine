...
def test_GradientBoost_basics():
    ...
    with StopWatch("Lucid Gradient compressed construction"):
        compressed_gbr = lucid_gbr.output_compressed_ensemble()
    with StopWatch("Lucid Gradient compressed Boost prediction"):
        compressed_gbr_pred = compressed_gbr.predict(X_df)

    ######################################################
    # test prediction outputted from LucidEnsemble
    np.testing.assert_almost_equal(compressed_gbr_pred, gbr_pred)
...
def test_RandomForest_basics():
    ...
    with StopWatch("Lucid Random Forest compressed construction"):
        compressed_rf = lucid_rf.output_compressed_ensemble()
    with StopWatch("Lucid Random Forest compressed Boost prediction"):
        compressed_rf_pred = compressed_rf.predict(X_df)

    ######################################################
    # test prediction outputted from LucidEnsemble
    np.testing.assert_almost_equal(compressed_rf_pred, rf_pred)
